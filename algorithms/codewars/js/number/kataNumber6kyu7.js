...
(except in C return NULL) and be ready 
for anything else which is not clearly specified ;)
*/


function tribonacci(signature, n) {
  const answer = signature.slice(0, n);

  for (let index = 0; index < n - signature.length; index += 1) {
    answer.push(answer[index] + answer[index + 1] + answer[index + 2]);
  }
  return answer;
}
