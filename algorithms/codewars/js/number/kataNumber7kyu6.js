...
You need to count the number of 1s in the binary 
representations of all the numbers in the array.
...
const rangeBitCount = (a, b) =>
  Array.from({ length: b - a + 1 }, (_, index) => index + a).reduce(
    (acc, value) => {
      return value.toString(2).replace(/0/g, "").length + acc;
    },
    0
  );
